...
def computeChrF(fpRef, fpHyp, nworder, ncorder, beta, sentence_level_scores = None):
    ...
    for hline, rline in zip(fpHyp, fpRef):
        ...
        # result for the no-match case
        bestMatchingCount = defaultdict(lambda: 0)
        bestRefCount = defaultdict(lambda: 0)
        bestHypCount = defaultdict(lambda: 0)
        bestChrMatchingCount = defaultdict(lambda: 0)
        bestChrRefCount = defaultdict(lambda: 0)
        bestChrHypCount = defaultdict(lambda: 0)
